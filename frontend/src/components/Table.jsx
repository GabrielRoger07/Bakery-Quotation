...
const Table = ({ title, columns = [], data = [], idKey = "id", loading = false, emptyMessage = "No records found.", onEdit, onDelete, onAdd, onView, onReload, onMonitor, onSort, sortField, sortDirection, toolbar, filterActive = false, filterSlot }) => {
    ...
    return (
        <>
        <div className="w-full max-w-[1200px] mx-auto mb-5 px-[1.625rem] py-6 bg-[var(--color-surface-card)] rounded-[var(--radius-xl)] border border-[var(--color-border-default)] [box-shadow:var(--shadow-card-soft)] hover:[box-shadow:var(--shadow-card-md)] transition-[box-shadow] duration-[160ms] max-sm:px-[0.75rem] max-sm:py-[0.875rem] max-sm:rounded-[var(--radius-md)] max-md:px-[1.125rem] max-md:rounded-[var(--radius-lg)]">
            ...
            <div className="flex justify-between items-center mb-5 max-sm:hidden max-md:flex-col max-md:items-start max-md:gap-[0.875rem]">
                <h1 className="m-0 text-[1.125rem] text-[var(--color-text-heading)] font-bold tracking-[-0.02em]">{title}</h1>
                    ...
                    {toolbar && (
                        <div className="relative inline-flex">
                            ...
                            {filterActive && !toolbarOpen && (
                                <span className="absolute top-[2px] right-[2px] w-[7px] h-[7px] rounded-full bg-[var(--color-accent)] border-[1.5px] border-[var(--color-surface-card)] pointer-events-none" />
                            )}
                        </div>
                    )}
            ...
            {toolbar && (
                <div className={`grid transition-[grid-template-rows] duration-[260ms] ease-[cubic-bezier(0.4,0,0.2,1)] ${toolbarOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
                    <div className="overflow-hidden">
                        <div className="flex gap-[0.625rem] items-center py-[0.875rem] pb-[calc(0.875rem+1.125rem)] border-t border-b border-[var(--color-border-faint)] max-sm:flex-wrap">
                            {toolbar}
                        </div>
                    </div>
                </div>
            ...
            {!loading && data.length > 0 && (
                <div className="overflow-x-auto overflow-y-visible [-webkit-overflow-scrolling:touch] rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)]">
                        ...
                        <tbody>
                            {data.map((item, index) => (
                                <tr key={item[idKey] || index} className="even:[&>td]:bg-[var(--color-surface-subtle)] hover:[&>td]:bg-[var(--color-highlight-lighter)] hover:[&>td]:transition-[background-color] hover:[&>td]:duration-[160ms]">
                                    ...
                                    {(onEdit || onDelete || onView || onMonitor) && (
                                        <td className="px-4 py-[0.875rem] border-b border-[var(--color-border-faint)] bg-[var(--color-surface-card)] max-md:px-3 max-md:py-[0.625rem]">
